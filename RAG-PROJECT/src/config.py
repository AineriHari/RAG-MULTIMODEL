...
class ConfigLoader:
    ...
    def set(self, *keys_and_value, force: bool = False) -> None:
        ...
        for key in keys[:-1]:
            if key not in current:
                if not self._allow_new_keys and not force:
                    raise KeyError(
                        f"Key path {'.'.join(keys)} does not exist and allow_new_keys is False. Use force=True to override."
                    )
                current[key] = {}
        ...
        if keys[-1] not in current and not self._allow_new_keys and not force:
            raise KeyError(
                f"Key '{keys[-1]}' does not exist in path {'.'.join(keys[:-1])} and allow_new_keys is False. Use force=True to override."
            )
    ...
    def _validate_key_exists(self, keys) -> None:
        ...
        for i, key in enumerate(keys):
            if not isinstance(current, dict) or key not in current:
                ...
                raise KeyError(
                    f"Key path '{key_path}' does not exist and allow_new_keys is False. Use force=True to override."
                )
            current = current[key]
    ...
    def get_pdf_pipeline_options(self) -> dict[str, Any]:
        ...
        vision_url = self.get(
            "model", "vision_url", default="http://localhost:11434/v1/chat/completions"
        )
        model_name = self.get("model", "vision_model", default="gemma3:4b")
        model_timeout = self.get("model", "timeout", default=60)
        picture_prompt = self.get(
            "document",
            "picture_description",
            "prompt_picture_description",
            default=(
                "Describe the image in detail. "
                "If the image contains text, charts, tables, or diagrams, "
                "extract and explain them precisely. "
                "Do not hallucinate."
            ),
        )
